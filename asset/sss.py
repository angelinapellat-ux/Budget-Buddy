...
class DashboardWindow(tk.Toplevel):
    ...
    def build_ui(self):
        ...
        self.create_entry(row1, "Description", self.vars["desc"], 15).pack(
            side="left", padx=(0, 10)
        )
        ...
        tk.OptionMenu(row2, self.vars["type"], "retrait", "dépôts", "transfert").pack(
            side="left", padx=(0, 10)
        )
        tk.OptionMenu(
            row2, self.vars["cat"], "Loisir", "Repas", "Facture", "Salaire", "Autre"
        ).pack(side="left")
        ...
        # --- COLONNE DROITE ---
        self.right = tk.Frame(
            main, bg="white", highlightthickness=1, highlightbackground="#E9ECEF"
        )
    ...
    def create_entry(self, master, label, var, width):
        ...
        tk.Label(
            f, text=label, bg="white", fg="#8898AA", font=("Segoe UI", 8, "bold")
        ).pack(anchor="w")
        tk.Entry(
            f,
            textvariable=var,
    ...
    def refresh_all(self):
        ...
        for t in self.user.get_filtered_transactions()[:15]:
            ...
            self.tree.insert(
                "",
                "end",
                values=(t[0], t[1], f"{signe}{t[2]:.2f} €", t[3].upper()),
                tags=(t[3],),
            )
    ...
    def save(self):
        try:
            ...
            if d and a > 0:
                ...
                if self.user.process_transaction(
                    ref,
                    d,
                    a,
